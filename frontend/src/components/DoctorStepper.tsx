    ...
    return (
        <div className="bg-white p-6 md:p-8 rounded-[32px] shadow-xl shadow-gray-200/50 border border-gray-100 max-w-2xl mx-auto mt-6 relative overflow-hidden">
            ...
            <div className="flex space-x-2 mb-8 max-w-sm mx-auto">
                <div className={`h-2 flex-1 rounded-full transition-all duration-500 ${step >= 1 ? 'bg-emerald-600 shadow-sm shadow-emerald-600/30' : 'bg-gray-100'}`} />
                <div className={`h-2 flex-1 rounded-full transition-all duration-500 ${step >= 2 ? 'bg-emerald-600 shadow-sm shadow-emerald-600/30' : 'bg-gray-100'}`} />
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-6">
                ...
                {step === 2 && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <textarea {...register('bio', { required: 'Professional Bio is required' })} rows={4} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 focus:bg-white focus:border-emeraldw-500 focus:ring-2 focus:ring-emeraldw-500/10 rounded-xl outline-none text-gray-900 text-sm font-medium transition-all shadow-sm resize-y leading-relaxed" placeholder="Tell patients about your expertise, background, and approach to medicine..."></textarea>
                        </div>
                    </div>
                )}
                ...
                <div className="flex items-center space-x-3 pt-4">
                    ...
                    <button type="submit" disabled={submitting} className="flex-1 px-6 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[15px] rounded-xl flex justify-center items-center shadow-lg shadow-emerald-600/20 hover:-translate-y-0.5 transition-all outline-none active:scale-[0.98]">
