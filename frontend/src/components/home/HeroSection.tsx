...
const HeroSection: React.FC<HeroSectionProps> = ({ onBookClick }) => {
    return (
        <div className="bg-white rounded-[20px] p-8 md:p-14 mb-14 shadow-sm border border-gray-100 flex flex-col lg:flex-row items-center gap-14 text-center lg:text-left relative overflow-hidden">
            <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-50 rounded-full -mr-32 -mt-32 blur-3xl opacity-50" />
            <div className="flex-1 lg:w-3/5 pl-0 lg:pl-10 relative z-10 text-left">
                <div className="inline-flex items-center space-x-2 bg-emerald-50 px-4 py-2 rounded-full border border-emerald-100 mb-8">
                    <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">MediBookAI Excellence</span>
                </div>
                ...
                <p className="text-gray-500 text-lg md:text-xl mb-10 max-w-xl text-left leading-relaxed font-bold italic">
            ...
            <div className="flex-1 lg:w-2/5 w-full relative flex items-center justify-center">
                <div className="absolute inset-0 bg-emerald-600/5 rounded-[20px] transform rotate-3 scale-105"></div>
                <img
                    src={doctorimage}
